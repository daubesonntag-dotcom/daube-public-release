...
export function validateProjection(projection) {
  ...
  if (!['candidate-not-integrated', 'verified-integrated'].includes(projection.upstreamState)) throw new Error('unsupported upstream state');
  if (projection.upstreamState === 'verified-integrated' && (!Array.isArray(projection.canonicalEvidenceRefs) || projection.canonicalEvidenceRefs.length < 3)) throw new Error('verified canonical upstream requires evidence refs');
  if (projection.productionAdoptionAllowed !== false) throw new Error('product production adoption requires a separate promotion contract');
  ...
  if (projection.truthBoundary?.canonicalUpstreamVerifiedDoesNotEqualProductProductionVerified !== true) throw new Error('canonical/product production truth boundary missing');
