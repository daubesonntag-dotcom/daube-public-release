...
def ensure_identity() -> tuple[str, str]:
    ...
    return public_pem, hashlib.sha256(public_pem.encode("utf-8")).hexdigest()
...
def cloud_heuristic() -> dict[str,object]:
    signals=read_first(["/sys/class/dmi/id/sys_vendor","/sys/class/dmi/id/product_name","/sys/class/dmi/id/board_vendor","/proc/device-tree/model"])
    joined=" | ".join(signals).lower()
    patterns=[("amazon-ec2",r"amazon ec2|amazon"),("google-compute",r"google compute engine"),("microsoft-azure",r"azure"),("oracle-cloud",r"oraclecloud|oracle cloud"),("digitalocean",r"digitalocean"),("hetzner",r"hetzner"),("vultr",r"vultr"),("linode",r"linode|akamai connected cloud")]
...
def storage_canary() -> dict[str,object]:
    ...
    with tempfile.NamedTemporaryFile(prefix="daube-sovereign-",dir=str(HOME),delete=False) as h:
        path=Path(h.name); h.write(payload); h.flush(); os.fsync(h.fileno())
...
def build_submission(public_pem: str, fingerprint: str) -> dict[str,object]:
    observed_at=time.strftime("%Y-%m-%dT%H:%M:%SZ",time.gmtime()); cloud=cloud_heuristic(); cpu=cpu_canary(); storage=storage_canary(); network=network_canary()
    canary={"success":bool(cpu["passed"] and storage["passed"] and network["passed"]),"observedAt":observed_at,"cpu":cpu,"storage":storage,"network":network,"privateAssetsUsed":False,"paidSpendAuthorized":False}
...
def main() -> int:
    require_runtime(); public_pem,fingerprint=ensure_identity(); submission=build_submission(public_pem,fingerprint)
    ...
    if status==409 and response.get("code") in ("host_key_not_registered","host_key_not_active"):
        print("PAIRING_REQUIRED: approve only this public-key fingerprint after confirming this is a directly controlled physical/local host."); return 3
    return 2
