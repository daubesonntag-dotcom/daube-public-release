...
class RuntimeSupervisor:
    def __init__(self,device):
        self.device=device; self.root=pathlib.Path(os.environ.get('DAUBE_RUNTIME_ROOT','/opt/daube/remote-commander/sovereign-runtime')); self.db=pathlib.Path(os.environ.get('DAUBE_RUNTIME_DB','/var/lib/daube/remote-commander/sovereign-runtime.db')); self.state=pathlib.Path('/var/lib/daube/remote-commander'); self.procs={}; self.last_start={}
    def specs(self):
        return {
          'worker':[sys.executable,str(self.root/'runtime.py'),'--db',str(self.db),'work','--worker',self.device,'--forever','--sleep','2'],
          'planner':[sys.executable,str(self.root/'planner-v1.py'),'--db',str(self.db),'--forever','--sleep','60']
        }
    def ensure(self):
        if not (self.root/'runtime.py').is_file(): return False
        for name,argv in self.specs().items():
            if name=='planner' and not (self.root/'planner-v1.py').is_file(): continue
            p=self.procs.get(name)
            if p is not None and p.poll() is None: continue
            if time.time()-self.last_start.get(name,0)<5: continue
            self.state.mkdir(parents=True,exist_ok=True); logf=open(self.state/f'sovereign-{name}.log','a',encoding='utf-8')
            try: self.procs[name]=subprocess.Popen(argv,cwd=str(self.root),env={**os.environ,'DAUBE_RUNTIME_DB':str(self.db)},stdout=logf,stderr=logf,start_new_session=True,close_fds=True); self.last_start[name]=time.time()
            finally: logf.close()
        return True
    def stop(self):
...
def main():
    ...
    try:
        while True:
            try:
                runtime.ensure(); jreq(rpc+'daube_remote_heartbeat',api_key=a.api_key,device=a.device,device_token=token,body={'p_device_id':a.device,'p_version':VERSION,'p_roots':[str(r) for r in roots]},timeout=20)
