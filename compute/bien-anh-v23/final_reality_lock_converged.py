...
def converge_scene(scene):
    ...
    if wall_mat and roof_mat:
        layers = [
            (2.05,-5.25,1.04,1.55,1.15,1.52,-2.2),(3.35,-4.85,.98,2.05,1.25,1.46,1.3),
            (4.65,-4.20,1.08,2.20,1.35,1.62,-1.0),(2.25,-3.05,.96,1.85,1.30,1.44,.7),
            (3.60,-2.55,1.05,2.30,1.40,1.58,-1.7),(4.85,-1.65,1.00,2.10,1.35,1.50,1.1),
            (2.20,-.70,1.02,1.80,1.25,1.53,-.8),(3.65,.05,1.08,2.25,1.40,1.62,1.5),
            (4.85,.95,.96,2.15,1.30,1.44,-1.2),(2.15,1.85,1.05,1.90,1.30,1.58,.8),
            (3.55,2.75,1.00,2.30,1.45,1.50,-1.4),(4.75,3.55,1.07,2.15,1.35,1.61,.9),
            (2.35,4.55,.98,1.85,1.25,1.47,-.6),(3.85,5.00,1.04,2.20,1.35,1.56,1.2),
        ]
        for i,(x,y,z,w,d,h,rz) in enumerate(layers,1):
            base.box(f"FINAL_SETTLEMENT_WALL_{i}",(x,y,h/2-.10),(w,d,h),wall_mat,.008,(0,0,math.radians(rz)))
            base.box(f"FINAL_SETTLEMENT_ROOF_{i}",(x,y,h+.035),(w+.30,d+.32,.052),roof_mat,.004,(0,math.radians(((i%3)-1)*1.1),math.radians(rz)))
