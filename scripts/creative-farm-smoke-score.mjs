...
const vfxOk=videoOk('vfx-composite.mp4',640,360,1.3);
add('media','vfx-composite',vfxOk,'VFX composite MP4 decodes at target dimensions/duration.');
...
const cgi=visual('cgi-frame.png'); const cv=(cgi?.streams||[]).find(s=>s.codec_type==='video'); const cgiOk=Boolean(cv&&Number(cv.width)===512&&Number(cv.height)===512&&exists('cgi-frame.png',10000));
add('media','cgi-frame',cgiOk,'Blender CGI frame rendered and decodes.');
...
add('quality','cgi-resolution',cgiOk,'CGI smoke frame is exactly 512x512.');
...
add('factory','code-to-motion',videoOk('motion.mp4',1280,720,1.8)&&videoOk('animation.mp4',640,360,1.3),'Media farm produced two valid temporal outputs.');
add('factory','code-to-vfx-cgi',vfxOk&&cgiOk,'VFX and Blender CGI lanes both produced valid decoded artifacts.');
