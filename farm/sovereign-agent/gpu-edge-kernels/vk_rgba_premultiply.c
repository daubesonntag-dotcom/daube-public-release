...
static int read_input(const char *path, uint32_t **words_out, uint32_t *count_out) {
  ...
  long size = ftell(f);
  if (size <= 0 || size > (long)(MAX_PIXELS * sizeof(uint32_t)) || (size % 4) != 0) { fclose(f); return 3; }
  rewind(f);
...
int main(int argc, char **argv) {
  if (argc != 3) {
    fprintf(stderr, "usage: daube-vulkan-rgba-premultiply INPUT_RGBA8_BIN OUTPUT_RGBA8_BIN\n");
    return 2;
  }

  uint32_t *input_words = NULL;
  uint32_t pixel_count = 0;
  int input_rc = read_input(argv[1], &input_words, &pixel_count);
  if (input_rc != 0) { fprintf(stderr, "input_invalid:%d\n", input_rc); return 3; }
  const VkDeviceSize byte_size = (VkDeviceSize)pixel_count * sizeof(uint32_t);
  ...
  app.pApplicationName = "D'AUBE Phone Edge RGBA";
  ...
  CHECK_VK(vkMapMemory(device, memory, 0, byte_size, 0, &mapped), "vkMapMemory");
  memcpy(mapped, input_words, (size_t)byte_size);
  ...
  VkDescriptorBufferInfo dbi = {buffer, 0, byte_size};
  ...
  vkCmdPushConstants(cmd, pipeline_layout, VK_SHADER_STAGE_COMPUTE_BIT, 0, sizeof(pixel_count), &pixel_count);
  vkCmdDispatch(cmd, (pixel_count + 63u) / 64u, 1, 1);
  ...
  if (write_output(argv[2], mapped, (size_t)byte_size) != 0) { fprintf(stderr, "output_write_failed\n"); rc = 8; goto cleanup; }

  printf("{\"schema\":\"daube.vulkan-rgba-premultiply-result.v1\",\"passed\":true,\"hardwareGpu\":true,\"softwareRenderer\":false,\"backend\":\"vulkan\",\"kernelId\":\"rgba-premultiply-u8-v1\",\"pixels\":%u,\"deviceName\":", pixel_count);
  json_string(chosen_props.deviceName);
  printf(",\"deviceType\":%u,\"vendorId\":%u,\"deviceId\":%u,\"apiVersion\":%u,\"queueFamily\":%u,\"computeQueue\":true}\n",
...
cleanup:
  ...
  if (pool != VK_NULL_HANDLE && device != VK_NULL_HANDLE) vkDestroyDescriptorPool(device, pool, NULL);
