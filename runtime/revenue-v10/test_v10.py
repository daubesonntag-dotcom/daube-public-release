...
class Fixture:
    ...
    def job(self, project_id=1, bid_id=2, status="AWARDED_ACCEPTED"):
        ...
        (directory / "job.json").write_text(
            json.dumps({"project_id": project_id, "bid_id": bid_id, "status": status})
        )
        return directory
...
class EvidenceTests(unittest.TestCase):
    def test_bid_not_revenue(self):
        fixture = Fixture()
        (fixture.roots["bid_receipts"] / "b.json").write_text(
            json.dumps({"authoritative": True, "project_id": 1, "bid_id": 2})
        )
    ...
    def test_mismatched_accept_fails_closed(self):
        fixture = Fixture()
        fixture.job()
        (fixture.roots["accept_receipts"] / "accept-1-2.json").write_text(
            json.dumps({"authoritative": True, "project_id": 999, "bid_id": 2})
        )
        self.assertEqual(
            evidence.canonical_state(evidence.resolve_project(1, fixture.roots)),
            "FAILED_CLOSED",
        )
        fixture.close()
    ...
    def test_delivery_then_pending_then_settled_precedence(self):
        fixture = Fixture()
        fixture.job()
        (fixture.roots["money_receipts"] / "delivery-1.json").write_text(
            json.dumps({"authoritative": True, "project_id": 1})
        )
        self.assertEqual(evidence.canonical_state(evidence.resolve_project(1, fixture.roots)), "DELIVERED")
        (fixture.roots["money_receipts"] / "milestone-release-1.json").write_text(
            json.dumps({"authoritative": True, "project_id": 1})
        )
        self.assertEqual(
            evidence.canonical_state(evidence.resolve_project(1, fixture.roots)),
        ...
        fixture.roots["revenue_ledger"].write_text(
            json.dumps(
                {
                    "project_id": 1,
                    "authoritative_external_settlement": True,
                    "evidence": "official_get_milestones_released_or_paid",
                    "amount": 100,
                }
            )
            + "\n"
    ...
    def test_fake_ledger_not_settled(self):
        ...
        fixture.job()
        fixture.roots["revenue_ledger"].write_text(
            json.dumps({"project_id": 1, "authoritative_external_settlement": False, "evidence": "test"})
            + "\n"
        )
...
class ConciergeTests(unittest.TestCase):
    def test_no_unsolicited(self):
        self.assertEqual(
            concierge.may_send("STATUS_UPDATE", False, {}, 100, None),
            (False, "NO_VERIFIED_RELATIONSHIP"),
        )

    def test_dedup(self):
        self.assertEqual(
            concierge.may_send("AWARD_ACK", True, {"AWARD_ACK": 1}, 100, None),
            (False, "DUPLICATE"),
        )

    def test_rate_limit(self):
        self.assertEqual(
            concierge.may_send("STATUS_UPDATE", True, {}, 10000, 9000),
            (False, "RATE_LIMIT"),
        )

    def test_new_activity_can_bypass_status_time(self):
        self.assertEqual(
            concierge.may_send("STATUS_UPDATE", True, {}, 10000, 9000, True),
            (True, "PASS"),
        )
...
class ControllerTests(unittest.TestCase):
    ...
    def test_atomic_and_one_service_per_cycle(self):
        ...
        for project_id in (1, 2):
            (fixture.roots["bid_receipts"] / f"{project_id}.json").write_text(
                json.dumps({"authoritative": True, "project_id": project_id, "bid_id": project_id + 10})
            )
    ...
    def test_secret_scrub(self):
        scrubbed = controller._scrub({"api_key": "abc", "nested": {"token": "def"}})
