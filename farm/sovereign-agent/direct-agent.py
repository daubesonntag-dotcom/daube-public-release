...
def disk_free_mib() -> int: return int(shutil.disk_usage(str(HOME)).free//(1024*1024))
...
def network_canary() -> dict[str,object]:
    req=urllib.request.Request(INTAKE_URL,headers={"Accept":"application/json","User-Agent":"daube-sovereign-agent/2"}); start=time.perf_counter()
    try:
...
def build_submission(public_pem: str, fingerprint: str) -> dict[str,object]:
    ...
    attestation={"schema":"daube.sovereign-direct-proof.v2","hostId":host_id,"observedAt":observed_at,"platform":"linux","runtimeKind":runtime_kind(),"arch":platform.machine(),"kernelRelease":platform.release(),"ownerControlClaim":True,"sensitiveDataIncluded":False,"privateAssetsUsed":False,"paidSpendAuthorized":False,"capacity":{"cpuLogical":os.cpu_count() or 0,"memoryMiB":memory_mib(),"diskFreeMiB":disk_free_mib(),"uptimeSeconds":uptime_seconds()},"cloudHeuristic":cloud,"identity":{"publicKeySha256":fingerprint,"hostnameSha256":hashlib.sha256(socket.gethostname().encode()).hexdigest()},"canary":canary}
    return {"schema":"daube.sovereign-direct-submission.v2","attestation":attestation,"publicKeyPem":public_pem,"signatureBase64":sign(attestation)}
def submit(payload: dict[str,object]) -> tuple[int,dict[str,object]]:
    req=urllib.request.Request(INTAKE_URL,data=json.dumps(payload,separators=(",",":"),ensure_ascii=False).encode(),headers={"Content-Type":"application/json","Accept":"application/json","User-Agent":"daube-sovereign-agent/2"},method="POST")
    try:
...
def main() -> int:
    ...
    summary={"schema":"daube.sovereign-direct-agent-result.v2","httpStatus":status,"status":response.get("status","UNKNOWN"),"code":response.get("code"),"hostId":submission["attestation"]["hostId"],"runtimeKind":submission["attestation"]["runtimeKind"],"publicKeySha256":fingerprint,"cloudDetected":submission["attestation"]["cloudHeuristic"]["detected"],"canaryPass":submission["attestation"]["canary"]["success"],"nextGate":response.get("nextGate")}; print(json.dumps(summary,indent=2,ensure_ascii=False))
    if status in (200,201) and response.get("status")=="VERIFIED": return 0
    if status==409 and response.get("code") in ("host_key_not_registered","host_key_not_active"): print("PAIRING_REQUIRED: approve only this public-key fingerprint after confirming this exact device/host is directly controlled by D'AUBE/founder."); return 3
    return 2
